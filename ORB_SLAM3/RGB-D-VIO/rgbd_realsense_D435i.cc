...
#include "lcmTypes/vio_cartesian_t.hpp"
...
int main(int argc, char **argv) {
    ...
    while (!SLAM.isShutDown())
    {
        ...
        Sophus::SE3f  T_ =SLAM.TrackRGBD(im, depth, timestamp); //, vImuMeas); depthCV
        cv::Mat pose = cv::Mat(4, 4, CV_32F, T_.matrix().data());
        lcmTypes::vio_cartesian_t my_data;
        cv::Mat rotationMatrix = pose(cv::Rect(0, 0, 3, 3));
...
//std::cout<<rpy[0]<<" "<<rpy[1]<<" "<<rpy[2]<<" "<<std::endl;
        for (int i = 0; i < 3; ++i) {
            my_data.pos[i] = translationVector[i];
            my_data.rot[i] = rpy[i];
        }

        lcm.publish("vio_out", &my_data);
